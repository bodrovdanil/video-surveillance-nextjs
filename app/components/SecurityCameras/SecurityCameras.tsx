...
const SecurityCameras = () => {
    return (
        <div className="bg-blue-red w-full text-center  text-white pt-1 ">
            ...
            <h2 className="text-[36px] font-semibold uppercase mt-[154px]">камеры <span className='font-extrabold'>видеонаблюдения</span></h2>
            <div className="flex flex-col flex-wrap items-center">
                {SecurityCamerasData.map((item: ISecurityCameras, index: number) => (
                    <div key={index} className={`w-[1155px] flex bg-no-repeat ${item.id % 2 === 0 ? 'flex-row-reverse' : 'flex-row'} relative`} style={{
                        ...
                        <div className={`flex-initial flex flex-col justify-center items-start text-start relative w-[570px] ${item.id % 2 === 0 ? 'pl-10': 'pr-10'} `}>
                            ...
                            <div className={`${item.id < 4 ? 'flex justify-end w-[510px]' : ''} `} >
